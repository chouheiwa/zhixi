...
export function ConversionAnalysis({ records }: Props) {
  ...
  return (
    <Card title="阅读量 vs 收益" size="small" extra={<span style={{ fontSize: 12, color: '#999' }}>平均 RPM ¥{avgRPM.toFixed(2)}</span>}>
      <ReactECharts option={option} style={{ height: 300 }} />
      <div style={{ fontSize: 11, color: '#999', textAlign: 'right', marginTop: 4 }}>
        每个点是一篇内容 | 红色虚线 = 同等阅读量下的最高收益线
      </div>
      <FormulaBlock title="" items={[
        { name: '效率前沿线', formula: '按阅读量从小到大排序\n保留每个阅读量级别上收益最高的点\n连线即为效率前沿', desc: '前沿线上的内容是"同等阅读量下赚钱最多的"。你的内容离前沿线越近，说明变现效率越高。远低于前沿线的内容可能有优化空间。' },
      ]} />
    </Card>
  );
}
