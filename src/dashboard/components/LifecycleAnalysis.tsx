...
export function LifecycleAnalysis({ incomeRecords }: Props) {
  const analysis = useMemo(() => {
    ...
    let cum = 0;
    for (const v of incomes) { cum += v; cumulative.push(cum); }
    ...
    return { incomes, dates, rpms, expFit, powFit, multiplier, cumulative, expCurve, powCurve, earlySum, totalSum, firstNDays };
  }, [incomeRecords]);
  ...
  const decayOption = {
    ...
    legend: { data: ['实际收益', ...(analysis.expFit ? ['趋势线'] : []), ...(analysis.powFit ? ['长尾趋势'] : [])], textStyle: { fontSize: 11 }, right: 0, top: 0 },
  ...
  const cumulativeOption = {
    ...
    xAxis: { type: 'category' as const, data: dates, axisLabel: { fontSize: 10 }, axisTick: { show: false } },
    yAxis: [
      { type: 'value' as const, axisLabel: { fontSize: 10, formatter: (v: number) => `¥${v.toFixed(0)}` }, splitNumber: 3, position: 'left' as const },
      { type: 'value' as const, axisLabel: { fontSize: 10, formatter: (v: number) => `¥${v.toFixed(1)}` }, splitNumber: 3, position: 'right' as const },
    ],
    series: [
      {
        name: '累计总收益',
  ...
  const bestFit = analysis.expFit && analysis.powFit
    ? (analysis.expFit.r2 >= analysis.powFit.r2 ? 'exp' : 'pow')
    : analysis.expFit ? 'exp' : 'pow';
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      {lifecycleDesc && (
        <div style={{ fontSize: 12, color: '#666', marginBottom: 12 }}>{lifecycleDesc}</div>
      )}

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 10, marginBottom: 16 }}>
        {analysis.expFit && (
          <>
            <MiniCard label="收益减半时间"
            ...
            <MiniCard label="预估总收益"
              value={`¥${analysis.expFit.ltv.toFixed(2)}`}
              sub="按当前衰减速度推算" />
          </>
        )}
        {analysis.powFit && (
          <MiniCard label="长尾程度"
            ...
            sub={analysis.powFit.alpha < 0.5 ? '老内容仍有持续收益' : analysis.powFit.alpha < 1 ? '收益逐渐减少' : '收益很快归零'}
            highlight={bestFit === 'pow'} />
        )}
        <MiniCard label="前7天 vs 总收益"
      ...
      <FormulaBlock title="本区域使用的计算方法" items={[
        { name: '收益减半时间（指数衰减拟合）', formula: '收益(t) = A × e^(-λt)\n半衰期 = ln(2) ÷ λ\n预估总收益 = A ÷ λ', desc: '假设收益按固定比例逐日递减（就像放射性衰变一样）。A是初始收益强度，λ是衰减速度。对ln(收益)和天数做线性回归来求A和λ。半衰期就是收益降到一半所需的天数。' },
        { name: '长尾程度（幂律衰减拟合）', formula: '收益(t) = A × t^(-α)\nα < 0.5: 强长尾  α > 1: 快速衰减', desc: '有些内容不是指数衰减，而是"开始降得快，后面降得慢"，老文章还能细水长流。α越小长尾效应越强。对ln(收益)和ln(天数)做线性回归来求α。' },
        { name: '前7天倍数', formula: '倍数 = 总收益 ÷ 前7天收益', desc: '一个简单的经验指标：如果你的内容前7天赚了¥10，倍数是3倍，那总收益大约是¥30。可以用历史内容的平均倍数来预测新内容的总收益。' },
      ]} />
