    ...
    <span style={{ position: 'relative', display: 'inline-flex', alignItems: 'center' }}>
      <span
        ...
        style={{
          ...
          background: open ? themeColors.warmBlue : themeColors.border, color: open ? '#fff' : '#999',
          cursor: 'pointer', userSelect: 'none', marginLeft: 4, flexShrink: 0,
        }}
      >
        ?
      </span>
      {open && (
        <div style={{
          ...
          background: '#fff', border: `1px solid ${themeColors.border}`, borderRadius: 8,
          padding: '10px 14px', width: 320, boxShadow: '0 4px 12px rgba(0,0,0,0.1)',
        }}>
          <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 6, color: '#333' }}>计算公式</div>
          <div style={{
            fontFamily: 'monospace', fontSize: 11, background: themeColors.paper,
            padding: '6px 10px', borderRadius: 4, marginBottom: 8, whiteSpace: 'pre-wrap',
...
export function FormulaBlock({ title, items }: {
  title: string;
  ...
  return (
    <div style={{ marginTop: 8 }}>
      ...
      {open && (
        <div style={{
          marginTop: 6, background: '#f8f9fa', borderRadius: 6,
          ...
          {items.map((item, i) => (
            <div key={i} style={{ marginBottom: i < items.length - 1 ? 8 : 0 }}>
              ...
              <div style={{
                fontFamily: 'monospace', fontSize: 11, color: themeColors.warmBlue,
                background: '#fff', padding: '3px 8px', borderRadius: 3,
