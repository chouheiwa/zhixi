...
function describeTrend(recent7: number[], prior7: number[]): { text: string; color: string } {
  ...
  if (priorAvg === 0) return { text: '→ 平稳', color: themeColors.muted };
  const change = (recentAvg - priorAvg) / priorAvg;
  if (change > 0.2) return { text: '↑ 上升', color: themeColors.sage };
  if (change < -0.2) return { text: '↓ 下降', color: themeColors.warmRed };
  return { text: '→ 平稳', color: themeColors.muted };
}
...
export function EngagementEfficiencyChart({ dailyRecords }: Props) {
  const { chartOption, trends } = useMemo(() => {
    ...
    const trendResults: { label: string; text: string; color: string }[] = [];

    for (const m of RATE_METRICS) {
      const rates = sorted.map((r) => (r.pv > 0 ? (m.getter(r) / r.pv) * 100 : null));
      const ma7 = movingAvg(rates, 7);
        ...
        formatter: (params: { seriesName: string; value: number | null; marker: string; axisValue: string }[]) => {
          ...
          for (const p of params) {
            if (p.value != null && !p.seriesName.includes('均线')) {
              const idx = sorted.findIndex((r) => r.date.slice(5) === params[0].axisValue);
              const record = idx >= 0 ? sorted[idx] : null;
      ...
      extra={
        <Flex gap={4}>
          {trends.map((t) => (
            <Tag
              key={t.label}
              color={t.color === themeColors.sage ? 'green' : t.color === themeColors.warmRed ? 'red' : 'default'}
              style={{ fontSize: 11 }}
            >
              {t.label} {t.text}
            </Tag>
