  ...
  const option = {
    tooltip: {
      ...
      formatter: (params: any[]) => {
        ...
        const lines = params.map((p: any) => `${p.seriesName}: ${p.seriesName === '平均收益' ? `¥${p.value.toFixed(2)}` : p.value.toFixed(0)}`);
  ...
  return (
    <Card title="哪天赚得多？" size="small">
      ...
      <FormulaBlock title="" items={[
        { name: '星期效应', formula: '某天平均收益 = Σ(该星期几所有天的收益) ÷ 该星期几出现的天数', desc: '把所有数据按星期几分组，分别求平均值。样本越多越准。颜色越深代表收益越高。' },
      ]} />
    </Card>
  );
}
