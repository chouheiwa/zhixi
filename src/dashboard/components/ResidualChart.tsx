    ...
    <div style={{ marginBottom: 24 }}>
      ...
      <div style={{ fontSize: 12, color: '#666', marginBottom: 12 }}>
        ...
        {analysis.anomalies.length > 0 &&
          ` 有 ${analysis.anomalies.length} 天的收益明显偏离预测（高亮标记），可能受平台推荐算法影响。`}
      </div>
        ...
        items={[
          {
            name: '预测模型（岭回归）',
            formula:
              '预测收益 = b₀ + b₁×阅读 + b₂×点赞 + b₃×评论 + b₄×收藏 + b₅×分享\n系数通过最小化预测误差 + 正则化惩罚求得',
            desc: '用这篇内容每天的各项指标（阅读、点赞等）来预测当天应该赚多少钱。如果预测和实际差距大，说明有其他因素（如平台推荐权重、内容质量分）在影响收益。',
          },
          {
            name: '准确度（R² 决定系数）',
            formula: 'R² = 1 - Σ(实际-预测)² ÷ Σ(实际-均值)²\n范围：0~100%',
            desc: 'R²=80%意味着80%的收益变化可以被指标解释，剩下20%是"运气"或其他因素。R²越高，说明收益和指标关系越紧密。',
          },
          {
            name: '异常点检测',
            formula: 'Z = (残差 - 残差均值) ÷ 残差标准差\n|Z| ≥ 1.5 判定为异常',
            desc: '残差=实际-预测。如果某天残差特别大，说明那天的收益无法被指标解释，可能受到了推荐加权、热搜等外部因素影响。',
          },
        ]}
      />
