...
export function GlobalCorrelationAnalysis({ records }: Props) {
  ...
  const aggregated = useMemo(() => {
    ...
    for (const r of dailyData) {
      ...
      if (existing) {
        existing.pv += r.pv; existing.show += r.show; existing.upvote += r.upvote;
        existing.comment += r.comment; existing.collect += r.collect; existing.share += r.share;
      } else {
        dailyMap.set(r.contentToken, {
          pv: r.pv, show: r.show, upvote: r.upvote,
          comment: r.comment, collect: r.collect, share: r.share, title: r.title,
        });
      }
    }
  ...
  const analysis = useMemo(() => {
    ...
    const weights = METRIC_INFO.map(({ key, label, color }, i) => ({
      key, label, color, weight: regression.coefficients[i + 1],
    ...
    const elasticities = METRIC_INFO.map(({ key, label, color }, i) => ({
      key, label, color, elasticity: elastic.elasticities[i], r2: elastic.r2s[i],
    ...
    const ridgeWeights = METRIC_INFO.map(({ key, label, color }, i) => ({
      key, label, color, weight: ridge.coefficients[i + 1],
    })).sort((a, b) => Math.abs(b.weight) - Math.abs(a.weight));

    const interaction = interactionRegression(xs, incomeValues, METRIC_INFO.map(m => m.label));
    ...
    return {
      correlations, regression, weights, contributions, elasticities,
      ridge, ridgeWeights, interaction, topInteractions, quantiles, residuals,
      contentCount: aggregated.length, topContrib,
    };
  }, [aggregated]);
  ...
  const readInterAnalysis = useMemo(() => {
    ...
    for (const r of records) {
      ...
      if (e) { e.read += r.currentRead; e.interaction += r.currentInteraction; e.income += r.currentIncome; }
      else map.set(r.contentId, { read: r.currentRead, interaction: r.currentInteraction, income: r.currentIncome });
    }
    const items = Array.from(map.values()).filter(i => i.read > 0).map(i => ({
      ...i, income: i.income / 100,
    ...
    return {
      ...
      tiers: [
        { label: '互动率低', engRate: tierAvgEng(tiers.low), income: avg(tiers.low), rpm: avgRpm(tiers.low), count: tiers.low.length },
        { label: '互动率中', engRate: tierAvgEng(tiers.mid), income: avg(tiers.mid), rpm: avgRpm(tiers.mid), count: tiers.mid.length },
        { label: '互动率高', engRate: tierAvgEng(tiers.high), income: avg(tiers.high), rpm: avgRpm(tiers.high), count: tiers.high.length },
      ],
    };
  }, [records]);
  ...
  const inclusionDelay = useMemo(() => {
    ...
    for (const r of allIncomeRecords) {
      ...
      if (existing) {
        ...
        map.set(r.contentId, { publishDate: r.publishDate, firstRecordDate: r.recordDate, title: r.title, contentType: r.contentType });
      }
    }
    ...
    <div>
      <AntCard size="small" style={{ marginBottom: 16 }}>
        <div style={{ fontSize: 13, color: '#666' }}>
          综合分析了 <b>{analysis.contentCount}</b> 篇内容的数据。
          模型{fitQuality}（准确度 {(analysis.ridge.r2 * 100).toFixed(0)}%）。
          {analysis.topContrib && <span> 目前<b>{analysis.topContrib.label}</b>对收益贡献最大（{analysis.topContrib.pct.toFixed(0)}%）。</span>}
        </div>
      </AntCard>
      ...
      <Row gutter={[16, 16]}>
        ...
        <Col span={12}><Card title="哪些指标和收益关系最大？" subtitle="柱子越长 = 关系越紧密，靠右 = 正向关系">
        ...
        {/* What drives income? */}
        <Col span={12}><Card title="收益主要靠什么？" subtitle="各指标对收益的贡献比例">
          {analysis.contributions.map(({ key, label, color, pct }) => (
            <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
              <div style={{ width: 50, fontSize: 12, color: '#666' }}>{label}</div>
              <div style={{ flex: 1, height: 16, background: '#eee', borderRadius: 8, overflow: 'hidden' }}>
                <div style={{ width: `${Math.min(pct, 100)}%`, height: '100%', background: color, borderRadius: 8 }} />
              </div>
        ...
        <Col span={12}><Card title="每个指标值多少钱？" subtitle="每增加1个阅读/点赞/评论等，收益增加多少">
        ...
        <Col span={12}><Card title="提升哪个指标最划算？" subtitle="该指标增长10%时，收益大约增长多少">
            ...
            <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
              ...
              <div style={{ flex: 1 }}>
                <span style={{
                  ...
                  {elasticity > 0 ? '+' : ''}{(elasticity * 10).toFixed(1)}%
                </span>
              </div>
              <div style={{ fontSize: 11, color: '#666' }}>
                {describeElasticity(elasticity)}
              </div>
            </div>
          ))}
        </Card></Col>
        ...
        {/* Which combos work? */}
        <Col span={12}><Card title="哪些指标组合更赚钱？" subtitle="同时提升这两个指标时，是否有额外加成">
          {analysis.topInteractions.length === 0 ? (
            <div style={{ color: '#999', fontSize: 12, textAlign: 'center', padding: 10 }}>未发现明显的指标组合效应</div>
          ) : (
        ...
        <Col span={12}><Card title="收益的最好和最差情况" subtitle="每增加1单位指标时，乐观和悲观的收益变化">
        ...
        {/* Surprising content */}
        <Col span={12}><Card title="哪些内容的收益出乎意料？" subtitle={`模型预测平均偏差 ${analysis.residuals.mape.toFixed(0)}%`}>
          ...
          {aggregated
            ...
            .map((item, i) => (
              <div key={i} style={{
                ...
                <div style={{ fontSize: 11, marginBottom: 3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {item.title}
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#666' }}>
                  <span>预期 ¥{item.predicted.toFixed(2)} | 实际 ¥{item.actual.toFixed(2)}</span>
        ...
        {readInterAnalysis && (
          <Col span={12}><Card title="阅读+互动如何影响收益？" subtitle="按阅读量和互动量分成4组对比">
        ...
        {readInterAnalysis && (
          <Col span={12}><Card title="互动率高是不是更赚钱？" subtitle="按互动率（互动÷阅读）分成三组对比">
            {readInterAnalysis.tiers.map((t, i) => (
              <div key={i} style={{ marginBottom: 10 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 3 }}>
                  <span style={{ color: '#666' }}>{t.label}（{t.engRate.toFixed(1)}%）· {t.count}篇</span>
                </div>
        ...
        {/* Inclusion delay */}
        {inclusionDelay && (
          <Col span={12}><Card title="发布多久后开始有收益？" subtitle={`统计了采集期内发布的 ${inclusionDelay.total} 篇内容`}>
            <div style={{ display: 'flex', gap: 12, marginBottom: 12 }}>
              ...
              <div style={{ flex: 1, background: '#f5f5f5', borderRadius: 6, padding: '8px 10px', textAlign: 'center' }}>
                ...
                <div style={{ fontSize: 14, fontWeight: 700, color: '#333' }}>{inclusionDelay.min}天 / {inclusionDelay.max}天</div>
              </div>
            </div>
            ...
            {inclusionDelay.slowest.length > 0 && inclusionDelay.max > 7 && (
              <div style={{ marginTop: 8 }}>
                ...
                {inclusionDelay.slowest.slice(0, 3).map((item, i) => (
                  <div key={i} style={{ fontSize: 11, color: '#666', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginBottom: 2 }}>
      ...
      <FormulaBlock title="本区域使用的计算方法" items={[
        { name: '相关性分析（Pearson 相关系数）', formula: 'r = Σ(x-x̄)(y-ȳ) ÷ √[Σ(x-x̄)² × Σ(y-ȳ)²]\n范围：-1 到 +1', desc: '衡量两个指标之间的线性关系强度。+1表示完全正相关（一个涨另一个也涨），-1表示完全负相关，0表示没关系。我们用每篇内容的指标总值和总收益来计算。' },
        { name: '贡献占比', formula: '贡献_i = 权重_i × 指标_i的均值\n占比_i = 贡献_i ÷ Σ所有贡献 × 100%', desc: '先用回归算出每个指标的权重，再乘以该指标的平均值，得到"该指标对收益的绝对贡献"，最后归一化为百分比。' },
        { name: '每个指标值多少钱（岭回归 Ridge）', formula: '收益 = b₀ + b₁×阅读 + b₂×点赞 + b₃×评论 + ...\n求解：β = (X\'X + λI)⁻¹X\'y  (λ=1.0)', desc: '找一条最佳拟合线，使得"由指标预测的收益"和"实际收益"的误差最小。加入λ惩罚项（岭回归）是为了避免阅读量和曝光量这种高度相关的指标互相干扰，让结果更稳定。b₁就是每多一次阅读增加的收益。' },
        { name: '提升哪个指标最划算（弹性分析）', formula: 'ln(收益) = a + b × ln(指标)\n弹性 = b（即指标变1%，收益变b%）', desc: '对收益和指标同时取对数再做回归。得到的斜率b就是弹性：指标每增长1%，收益增长b%。这比直接看金额更公平，因为消除了量级差异（点赞100和阅读10000不在同一量级）。' },
        { name: '指标组合效应（交互项回归）', formula: '收益 = ... + b_ij × 指标_i × 指标_j + ...\nb_ij > 0 表示协同加成', desc: '在回归中加入两个指标的乘积项。如果系数为正，说明这两个指标同时高的内容，收益比单独高某一个指标要多（1+1>2的效果）。' },
        { name: '最好/最差情况（分位数回归）', formula: '最差(P10): 最小化 Σ 0.1×|正残差| + 0.9×|负残差|\n一般(P50): 最小化 Σ 0.5×|残差| (中位数回归)\n最好(P90): 最小化 Σ 0.9×|正残差| + 0.1×|负残差|', desc: '普通回归预测的是平均值，但实际收益有波动。分位数回归分别预测"运气差时(P10)"、"一般情况(P50)"、"运气好时(P90)"的收益。P10到P90的范围就是你大概率会落入的收益区间。' },
        { name: '出乎意料的内容（残差分析）', formula: '残差 = 实际收益 - 预测收益\nMAPE = Σ|残差/实际值| ÷ n × 100%', desc: '残差就是模型预测不了的部分。残差大的内容说明有我们没捕捉到的因素在影响收益（比如知乎的推荐算法加权、内容垂直度等）。MAPE是平均预测误差的百分比。' },
        { name: '阅读+互动分组', formula: '阅读量中位数 = 排序后取中间值\n高于中位数 = "高"，低于 = "低"\n分成4组后求各组平均收益', desc: '中位数不受极端值影响（一篇爆款不会拉高标准）。4组对比可以看出：阅读量和互动量哪个对收益影响更大，以及两者是否有协同效应。' },
        { name: '互动率分组', formula: '互动率 = 互动次数 ÷ 阅读次数 × 100%\n按互动率排序后均分三组\n千次阅读收益 = 收益 ÷ 阅读量 × 1000', desc: '千次阅读收益排除了阅读量差异，如果互动率高的组千次阅读收益也高，说明互动对收益有独立的加成作用，不只是因为阅读多。' },
        { name: '收录延迟', formula: '延迟天数 = 首次产生收益的日期 - 内容发布日期\n只统计：收益 > 0 且发布日期 ≥ 采集起始日期', desc: '只统计采集期内发布的内容，因为更早发布的内容我们没有完整的收益记录，无法确定真正的首次收益日期。"当天"表示发布当天就有收益（上午发布，当天就有阅读和收入）。' },
      ]} />
...
function Card({ title, subtitle, children }: {
  ...
  return (
    <AntCard size="small" title={title} extra={subtitle ? <span style={{ fontSize: 11, color: '#999', fontWeight: 400 }}>{subtitle}</span> : undefined}>
      {children}
    </AntCard>
  );
}
