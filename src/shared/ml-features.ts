...
export const FEATURE_NAMES = [
  ...
  'log_pv', 'log_show', 'log_upvote', 'log_comment', 'log_collect',
  'engagementRate', 'upvoteRate', 'commentRate', 'collectRate',
  'pvSquared', 'upvoteSquared',
  'pvXupvote', 'pvXcomment',
  'dayOfWeek_sin', 'dayOfWeek_cos',
  'contentAge', 'log_contentAge',
  'pv_ma3', 'log_pv_ma3', 'income_lag1', 'log_income_lag1',
];
  ...
  for (const [token, records] of byContent) {
    ...
    for (let i = 0; i < sorted.length; i++) {
      ...
      rows.push({
        features: [
          pv, show, upvote, comment, collect, share,
          safeLog(pv), safeLog(show), safeLog(upvote), safeLog(comment), safeLog(collect),
          engagementRate, upvoteRate, commentRate, collectRate,
          pvSquared, upvoteSquared,
          pvXupvote, pvXcomment,
          dayOfWeek_sin, dayOfWeek_cos,
          contentAge, safeLog(contentAge),
          pv_ma3, safeLog(pv_ma3), income_lag1, safeLog(income_lag1),
        ],
 ...
 */
export function buildContentFeatureRows(
  incomeRecords: IncomeRecord[],
): FeatureRow[] {
  const map = new Map<string, {
    read: number; interaction: number; income: number;
    publishDate: string; contentId: string; contentToken: string;
    dayCount: number;
  }>();
  ...
  for (const r of incomeRecords) {
    ...
    if (e) {
      ...
      map.set(r.contentId, {
        read: r.currentRead, interaction: r.currentInteraction,
        income: r.currentIncome, publishDate: r.publishDate,
  ...
  return Array.from(map.values()).filter(i => i.read > 0 && i.income > 0).map(item => {
    ...
    return {
      features: [
        item.read, item.interaction, engRate,
        readPerDay, incomePerDay, item.dayCount,
