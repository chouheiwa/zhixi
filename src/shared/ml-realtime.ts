...
export const REALTIME_FEATURE_NAMES = [
  'pv', 'show', 'upvote', 'comment', 'collect', 'share',
  'like', 'play', 'reaction', 'rePin',
  'newIncrUpvoteNum', 'newDescUpvoteNum', 'newIncrLikeNum', 'newDescLikeNum',
  'clickThroughRate', 'engagementRate', 'upvoteChurnRate',
  'dayOfWeek_sin', 'dayOfWeek_cos',
  'pvXupvote', 'pvXcomment',
  'yesterdayIncome',
];

export const REALTIME_FEATURE_LABELS: Record<string, string> = {
  pv: '阅读量', show: '曝光量', upvote: '净点赞', comment: '评论',
  collect: '收藏', share: '分享', like: '喜欢', play: '播放',
  reaction: '表情反应', rePin: '转发',
  newIncrUpvoteNum: '新增赞同', newDescUpvoteNum: '取消赞同',
  newIncrLikeNum: '新增喜欢', newDescLikeNum: '取消喜欢',
  clickThroughRate: '点击率', engagementRate: '互动率',
...
function extractFeatures(
  ...
  const upvoteChurnRate = record.newIncrUpvoteNum > 0
    ? record.newDescUpvoteNum / record.newIncrUpvoteNum
    : 0;
  ...
  return [
    pv, show, upvote, comment, collect, share,
    record.like, record.play, record.reaction, record.rePin,
    record.newIncrUpvoteNum, record.newDescUpvoteNum,
    record.newIncrLikeNum, record.newDescLikeNum,
    clickThroughRate, engagementRate, upvoteChurnRate,
    dayOfWeek_sin, dayOfWeek_cos,
    pvXupvote, pvXcomment,
    yesterdayIncome,
 ...
 */
export function buildRealtimeTrainingRows(
...
export function buildTodayFeatures(
  todayRecord: RealtimeAggrRecord,
  yesterdayIncome: number,
): number[] {
...
export function trainRealtimeModel(
  ...
  const ensemblePred = testY.map((_, i) =>
    Math.max(0, rfWeight * rfPred[i] + ridgeWeight * ridgePred[i])
  );
 ...
 */
export function predictWithRealtimeModel(
  saved: SavedRealtimeModel,
  features: number[],
): number {
